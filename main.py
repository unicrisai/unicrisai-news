...
def generate_html(news_items, filename, page_title):
    ...
    <head>
        ...
        <style>
            ...
            .nav-link {{ font-size: 10px; tracking: 0.2em; text-transform: uppercase; font-weight: bold; margin-right: 20px; }}
            .active-nav {{ border-bottom: 2px solid black; padding-bottom: 4px; }}
        </style>
    </head>
    <body class="font-sans antialiased">
        <header class="max-w-2xl mx-auto pt-16 px-6">
            ...
            <nav class="mt-6 flex">
                <a href="index.html" class="nav-link {'active-nav' if filename=='index.html' else ''}">Latest</a>
                <a href="archive.html" class="nav-link {'active-nav' if filename=='archive.html' else ''}">Archive</a>
            </nav>
